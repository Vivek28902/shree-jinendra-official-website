      ...
      <div className="grid grid-cols-1 md:grid-cols-12 h-full">
        ...
        <div className="col-span-12 md:col-span-7 h-[60vh] md:h-full relative overflow-hidden bg-brand-grey/5 order-2">
          {heroSlides.map((slide, index) => (
            <div
              key={index}
              className={`absolute inset-0 z-0 transition-opacity duration-[2000ms] ease-in-out ${index === currentSlide ? 'opacity-100' : 'opacity-0'
                }`}
            >
              <img
                src={slide.image}
          ...
          <div className="absolute bottom-8 left-1/2 transform -translate-x-1/2 z-20 flex gap-3">
            {heroSlides.map((_, index) => (
              <button
                ...
                className={`h-1 rounded-full transition-all duration-700 ${index === currentSlide ? 'w-10 bg-white' : 'w-4 bg-white/40 hover:bg-white/80'
                  }`}
